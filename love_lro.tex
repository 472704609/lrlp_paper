...
This, in turn, occurs only when $s^*_\omega = \bar{s}$.
The implication of this result is that only the most expensive scenarios can be popped. 
...
\begin{proposition} \label{prop:pop}
	...
	A scenario $\omega$ for which $q_\omega = 0$ can only be popped if $s^*_\omega = \bar{s}$.
\end{proposition}
