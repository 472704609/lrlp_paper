...
\begin{align*}
	\min_{\x,\lambda,\mu,y^\omega} \ & \c\x + \mu + \rho \lambda + \lambda \sum_{\omega=1}^{n} q_\omega \phi^*\left(\frac{\k^\omega y^\omega - \mu}{\lambda}\right) \\
	\st \ & \x \in X, \y^\omega \in Y_\omega(x) \nonumber \\
	& \frac{\k^\omega y^\omega - \mu}{\lambda} \leq \lim_{t \rightarrow \infty} \frac{\phi(t)}{t}, \ \forall \omega \\
	& \lambda \geq 0.
\end{align*}
See Theorem 1 of \cite{bental2011robust} for a derivation of the dual problem, and also the proof of Proposition \ref{prop:pop}.
...
\begin{example}{CVaR}
	...
	\]
	is equivalent to the $\phi$-divergence constrained problem with
	\[
		\phi(t) = \
		\begin{cases}
...
\begin{example}[``Reverse'' CVaR]
	...
	\[
		...
		\end{cases}
	\]
	for $0 < \beta < 1$ is equivalent to minimizing
	\[
		\c\x + \beta \sup_\omega h_\omega(\x) + (1-\beta)\e{h(\x)},
	\]
	where the expectation is taken with respect to the nominal distribution $q$.
\end{example}
...
\begin{example}[Conbination CVaR and Expectation]
	...
	\[
		...
		\end{cases}
	\]
	for $\alpha,\beta \in (0,1)$ is equivalent to minimizing
	\[
