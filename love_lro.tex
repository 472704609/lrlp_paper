...
Each arc $(i,j) \in A$ and time period $t$ has a unit cost $c_{ijt}^x$, loss coefficient $0 \leq a_{ijt} \leq 1$, and bounds on the flow $l_{ijt}^x \leq x_{ijt} \leq u_{ijt}^x$.
...
In real life, many parameters are unknown, especially as we further look into the future. 
...
The water supplies and demands are assumed to known during the first stage of $P_1$ years and in the remaining $P_2$ years, they are uncertain and modeled using a discrete set of scenarios with a specific probability distribution.  
...
\end{align}
where the $\omega$ index indicates the $n$ second-stage scenarios with probabilities $p_\omega$, and
\begin{align}
