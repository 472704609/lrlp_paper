...
\begin{proof}{\sc Proof of Proposition \ref{prop:pop}.}
	...
	\begin{description}
		...
		\item[Case 3: $\frac{h_\omega(\x) - \mu}{\lambda} = \bar{s}$] places no restrictions on the value of $p_\omega$, since (\ref{eq:pop_proof_condition}) is identically zero, and hence allows for $p_\omega > 0$ (popping). \Halmos %\qedhere
	\end{description}
\end{proof}
...
To demonstrate the techniques discussed in this paper, we applied the decomposition method from Section \ref{sec:soln_algorithm} to a small test problem.
...
All results in this section were computed with the test problem APL1P, a power expansion problem with 5 independent random variables and 1280 realizations, and can be found in \cite{infanger1992monte}.
...
Table \ref{tb:numerical_results} shows a few select results for the 6-sample APL1P, choosing $\rho$ in accordance with an asymptotic $95\%$ confidence region.
Behavior is shown assuming one observation per scenario for divergences above the line, and with the most costly scenario unobserved below the line.
...
\begin{table}
	\TABLE
	{
		...
		Divergences above the line use a single observation of each scenario, while the the most costly scenario was unobserved below the line to demonstrate popping behavior.
...
Figure \ref{fig:suppress} shows how the worst-case distribution changes with $\rho$ for both the Modified $\chi^2$-Distance (left) and the Kullback-Leibler divergence (right).
...
\begin{figure}
	\FIGURE
	{%
		\includegraphics*[width=.5\textwidth]{images/mchi2_decrease_condition}%
		\includegraphics*[width=.5\textwidth]{images/burg_decrease_condition}%
	}
	{
