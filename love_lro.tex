...
First, we investigate how \plp\ might change with a single additional observation in Section \ref{ssec:value}, then as more and more data is gathered with asymptotic results in Section \ref{ssec:epiconvergence}.
...
Our main goal is to come up with simple conditions using the current solution. 
...
We would like to avoid resolving the problem; furthermore, we would like to come up with easy-to-check conditions depending on the nominal probabilities $q_\omega$, the worst-case probabilities $p_\omega^*$, and the number of observations $N$. 
Toward this end, we first provide a general result. 
Then, we provide simpler conditions for a subset of the $\phi$-divergences by using only $\q$, $\p^*$ and $N$. 
