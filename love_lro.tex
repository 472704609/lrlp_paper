...
We begin by formulating a multi-period generalized network flow model of Colorado River water allocation in Tucson.
The generalized network is defined by a set of nodes and directed arcs $(N,A)$ defining the network topology.
Water flows on arc $(i,j) \in A$ during time period $t$, $t = 1, \dots, P$, are represented by decisions $x_{ijt}$.
...
Stored water available at node $j$ at the beginning of time period $t$ is $s_{jt}$, with associated cost $c_{jt}^2$ and bounds $l_{jt}^s \leq s_{jt} \leq u_{jt}$.
...
The generalized network model has a formulation give by
\begin{align*}
...
One technique to deal with this is to replace the known distribution with an ``ambiguity'' set of distribution, i.e., a set of distributions which is believed to contain the true distribution.
...
The ambiguity set is defined by the distributions with sufficiently high empirical likelihood $\prod_{\omega=!}^n p_\omega^{N_\omega}$.
