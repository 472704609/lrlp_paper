...
The unknown true distribution can then be modeled as coming from a distribution $Dir(\beta)$ with $\beta_\omega = N_\omega + 1$, and $\gamma'$ chosen such that $\P^*\left\{ \prod_{\omega=1}^n p_\omega^{N_\omega} \geq \gamma' \prod_{\omega=1}^n (\hat{p}^N_\omega)^{N_\omega} \right\}$.
...
Let the probability space $(\Xi,{\cal F},\P^{\text{true}})$ be the space associated with taking infinitely many samples from the distribution $\ptrue$.
...
\begin{proposition} \label{prop:weak_conv}
	For all $\epsilon > 0$ and $\xi \in \Xi'$ there exists $N'$ such that $\forall N \geq N'$ $D_{KL}(\hat{p}^N,p) \leq \frac{1}{N} \log\left(\frac{1}{\gamma'}\right) \Rightarrow \max_\omega |p_\omega - \ptrue_\omega| \leq \epsilon$.
\end{proposition}

\begin{proof}
	Without loss of generality, we assume that $\ptrue_\omega > 0$ for all $\omega$.
	This is valid because $\ptrue_\omega = 0 \Rightarrow \hat{p}^N_\omega$, which implies $p_\omega = 0$ in the worst case distribution by (\ref{eq:kkt}).
	...
	For the next portion of the proof, we use the alternate form of the KL divergence
	\[
		D_{KL}(\hat{p}^N,p) = \sum_{\omega=1}^n \hat{p}^N_\omega \left( - \log \frac{p_\omega}{\hat{p}^N_\omega} + \frac{p_\omega}{\hat{p}^N_\omega} - 1 \right),
	\]
	and use the fact that each term in parentheses $\phi(t) = -\log t + t - 1$ is non-negative for $t \geq 0$.
	...
	\end{align*}
	where the first inequality comes by focusing the smallest probability and using the inequalities $\frac{a+\epsilon}{a} \geq 1 + \epsilon$ and $\frac{a-\epsilon}{a} \leq 1 - \epsilon$ for $a \leq 1$.
\end{proof}
