...
The $\phi$-divergence has been used to measure the ``distance'' between two non-negative vectors $p = (p_1, \dots, p_n)^T$ and $q = (q_1, \dots, q_n)^T$, especially when $p$ and $q$ are probability vectors, i.e., satisfying $\sum_{\omega=1}^n p_\omega = \sum_{\omega=1}^n q_\omega = 1$.
...
Throughout this paper we assume that both $p$ and $q$ are probability vectors, we can additionally assume without loss of generality that $\phi(t) \geq 0$.
...
\end{equation}
satisfies all criteria for a $\phi$-divergence \cite{bental1991certainty}, and has the property that $I_{\tilde{\phi}}(p,q) = I_\phi(q,p)$.
...
For all divergences, $\phi(t) = \infty$ for $t < 0$, and the value of the conjugate is listed only in its domain, i.e., $s$ such that $\phi^*(s) < \infty$.
...
\begin{proof}
	...
	\[
		f_N(\x,\mu,\kappa) = c\x + \mu + N\rho_N \kappa + \sum_{\omega = 1}^n N_\omega \left[ \kappa \phi^*\left(\frac{h_\omega(\x) - \mu}{N\kappa} \right) \right].
	\]
	...
	\[
		cx + \mu + (N+1)\rho_{N+1} \kappa + \sum_{\omega = 1}^n N'_\omega \left[ \kappa \phi^*\left(\frac{h_\omega(x) - \mu}{(N+1)\kappa} \right) \right],
	\]
	...
	\end{equation}
	Returning to the original variables, we get $s^\omega_{N+1} \frac{N}{N+1} \frac{h_\omega(\x^*_N) - \mu^*_N}{\lambda^*_N}$
\end{proof}
...
\begin{proposition} \label{prop:weak_conv}
	Let $p \neq \ptrue$.
	For all $\epsilon > 0$ and $\xi \in \Xi'$ there exists $N'$ such that $\forall N \geq N'$ $I_{\phi}(\hat{p}^N,p) \leq \frac{\rho_0}{N} \Rightarrow \max_\omega |p_\omega - \ptrue_\omega| \leq \epsilon$.
\end{proposition}
...
\begin{proof}
	...
	\begin{align*}
		I_{\phi}(\hat{p}^N,p) & = \sum_{\omega=1}^n \hat{p}^N_\omega \phi\left( \frac{p_\omega}{\hat{p}^N_\omega} \right) \\
		& = \bar{s} \sum_{\omega \in Z} p_\omega \mathbb{I}_{\bar{s} < \infty} + \sum_{\omega \notin X} \hat{p}^N_\omega \phi\left( \frac{p_\omega}{\hat{p}^N_\omega} \right) \\
		& \geq 0 + \min_{\omega \notin Z} \{\hat{p}^N_\omega\} \cdot \max_{\omega \notin Z} \left\{ \phi \left( \frac{p_\omega}{\hat{p}^N_\omega} \right) \right\} \\
		& \geq \min_{\omega \notin Z} \{\hat{p}^N_\omega\} \cdot \min\left\{ \phi\left(1+\frac{\epsilon}{2}\right), \phi\left(1-\frac{\epsilon}{2}\right) \right\} \\
...
\begin{align*}
	\dfrac{\partial h^\dagger_\omega}{\partial \x}(\xh,\mh,\lh) & = \phi^{*\prime}(s_\omega(\xh,\mu,\lh)) \pi^{*,\omega}B^\omega \\
	\dfrac{\partial h^\dagger_\omega}{\partial \mu}(\xh,\mh,\lh) & = -\phi^{*\prime}(s_\omega(\xh,\mu,\lh)) \\
	\dfrac{\partial h^\dagger_\omega}{\partial \lambda}(\xh,\mh,\lh) & = \phi^*(s_\omega(\xh,\mu,\lh)) - \phi^{*\prime}(s_\omega(\xh,\mu,\lh))s_\omega(\xh,\mu,\lh),
\end{align*}
where $s_\omega(\xh,\mh,\lh) = \frac{h_\omega(\x) - \mu}{\lambda}$.
...
\begin{align*}
	...
	\left( \begin{array}{ccc}
		\phi^{*\prime}(s_\omega(\xh,\mu,\lh)) \pi^{*,\omega}B^\omega, 
			 & -\phi^{*\prime}(s_\omega(\xh,\mu,\lh)), 
			 & \phi^*(s_\omega(\xh,\mu,\lh)) - \phi^{*\prime}(s_\omega(\xh,\mu,\lh))s_\omega(\xh,\mu,\lh)
	\end{array} \right) \\
...
After the subproblems $h^\dagger_\omega(\xh)$ are solved, it may be the case that $\frac{h_\omega(\xh)-\mh}{\lh} < \bar{s}$ for some $\omega$, rendering $\mh$ and $\lh$ infeasible.
...
In this problem, the second scenario (displayed in green) is the most costly, and thus the only candidate for popping if its nominal probability vanishes.
...
An example of a $\phi$-divergence that suppress, the Burg entropy, is given in Figure \ref{fig:pop}.
