...
\authorlist{Love and Bayraksan}%Heading

\abstractID{1198}% Fill in the web conference management system-assigned abstract ID.

\conference{Proceedings of the 2013 Industrial and Systems Engineering Research Conference\\A. Krishnamurthy and W.K.V. Chan, eds.}
...
The network model has a number of nodes representing available water supply from the Colorado River, water treatment plants, reservoirs, and water demand nodes. 
The arcs represent the conveyence system (pipes, etc.) that carry water between the nodes. 
There are a number of time periods considered in between which water is stored in reservoirs to meet future demands. 
...
To apply the likelihood robust optimization framework we will for now begin with a general two-stage stochastic linear program with recourse (SLP-2), and use it to form a two-stage likelihood robust linear program with recourse, which we denote as LRLP-2.
...
Figure \ref{fig:water_prob_decrease} shows the computed values of $\frac{N_L}{N}$ (dashed line) and the estimated probability that an additional sample will remove the worst-case distribution from the likelihood region, resulting in a lower-cost solution. %, analogous the results of Figure \ref{fig:prob_cost_decrease_gp}.
...
To see this, recall that low-population scenarios have lower costs and . . .  
%%  DAVID: Please write intutive explanation
