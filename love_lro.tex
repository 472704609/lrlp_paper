...
One early example is \cite{hall1970optimal}, who formulates a linear stochastic programming model for arid and semiarid water rights allocation in agriculture.
\cite{bishop1975optimization} provides an early study of a stochastic model of a water allocation system, including wastewater treatment and reuse.
...
Sections \ref{sec:classification} \ref{sec:value} review our classification of $\phi$-divergences from \cite{love2013phi} and a value-of-data property of the $\phi$-divergence program.
...
Figure \ref{fig:shortage_frequency_piecewise} shows the total shortage amount over the 41-year study period for each infrastructure configuration when using the piecewise water use fit described in Table \ref{tb:gpcd_piecewise_reg}.
...
The addition of a reverse osmosis (RO) facility offers little benefit.
Figure \ref{fig:shortage_frequency_linear} shows a similar pattern, although with smaller shortages due to the lower water demand per capita through most of the study period.
These shortage amounts are computed with the Burg $\phi$-divergence, but are dictated largely by the physical constraints of the system, and do not change substantially with different $\phi$ or $\rho$.
...
We find that GFDL-CM3 with the RCP4.5 results in the highest operating cost, while MIROC5 with RCP6.0 has the lowest cost.
...
We can see that, while the satellite wastewater treatment plant decreases the cost only by \$10-\$15 million, the IPR facility reduces the cost by an addition \$25 million over the 41-year time span.
...
Finally, we applied the value of data condition developed in Section \ref{sec:value} to the results of the water model.
...
\begin{table}
	\TABLE
	{
		An indication of which climate models satisfy the value of data condition from Section \ref{sec:value}.
