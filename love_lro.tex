...
The recent work of \cite{hukullback} studies similar problems with ambiguous uncertainty either in the objective or the constraints where the uncertainty sets are defined by the KL-divergence.
\cite{hukullback} differs from this work by considering a continuous distributions, and doesn't relate the nominal distribution to observational data.
...
Section \ref{sec:lrlp2} presents the extension of LRO model to a two-stage stochastic program with recourse.
...
This divergence is included because Hu and Hong \cite{hukullback} and Wang et.\ al.\ \cite{wang2010likelihood} both use it to formulate a distributionally robust program.
...
Ben-Tal et.\ al.\ \cite{bental2011robust} proposed that the ambiguity set could be generated with $\phi$-divergences, including those distributions whose divergence from the nominal distribution $q$ is sufficiently small.
...
\begin{table}
	...
	\begin{tabular}{l|p{.3\textwidth}p{.3\textwidth}}
		 ...
		 \hline
		 Can Pop Scenarios %
			& \parbox{.3\textwidth}{Hellinger,\\Variation Distance} %
			& \parbox{.3\textwidth}{Burg Entropy,\\$\chi^2$-distance} \smallskip \\
...
To be consistent with the known $\phi$-divergence results stated in Section \ref{sec:robust_level}, we assume $\rho_N = \frac{\rho_0}{N}$.
...
\begin{align*}
	...
	\left( \begin{array}{ccc}
		\phi^{*\prime}(s_\omega(\xh,\mh,\lh)) \pi^{*,\omega}B^\omega, 
			 & -\phi^{*\prime}(s_\omega(\xh,\mh,\lh)), 
			 & \phi^*(s_\omega(\xh,\mh,\lh)) - \phi^{*\prime}(s_\omega(\xh,\mh,\lh))s_\omega(\xh,\mh,\lh)
	\end{array} \right) \\
	t_j^\omega & = \lh \phi^{*\prime}(s_\omega(\xh,\mh,\lh))\left[s_\omega(\xh,\mh,\lh) - \frac{\pi^{*,\omega}B^\omega\xh - \mh}{\lh}\right].
\end{align*}
...
\begin{figure}
	\begin{subfigure}{.5\textwidth}
		\includegraphics[width=\textwidth]{images/mchi2}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\includegraphics[width=\textwidth]{images/kl}
	\end{subfigure}
	\caption{
...
\begin{figure}
	\begin{subfigure}{.5\textwidth}
		\includegraphics[width=\textwidth]{images/burg}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\includegraphics[width=\textwidth]{images/burg_zero}
	\end{subfigure}
	\caption{
