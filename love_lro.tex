...
\begin{theorem}
	...
	Then, the optimal value of \plp\ given in (\ref{eq:plp_two_stage}) converges to that of SLP-2 given in (\ref{eq:slp_first_stage}) with $\qtrue$, and all limit points of the solutions $\x^*$ of \plp\ solve SLP-2 with $\qtrue$ as $N \rightarrow \infty$.
\end{theorem}

\begin{proof}{\sc Proof of Theorem \ref{thm:epiconvergence}.}
	The assumptions ensure $\sup_{\omega, \x \in X}|h_\omega(\x)|<C$ for some constant $C<\infty$. 
	This, together with $X\neq \emptyset$ and $\mathcal{P} \neq \emptyset$ both being compact, implies w.p.1 \plp\ and SLP-2 with $\qtrue$ have finite optimal solutions. 
   ...
   Let $f(\x,\omega)=\c\x + h_\omega(\x)$. 
	View the objective of \plp\ as $\ee{\p^*}{f(\x,\omega)}=\sum_{\omega=1}^{n}p^*_\omega f(\x,\omega)$ and the objective of SLP-2 with $\qtrue$ as $\ee{\qtrue}{f(\x,\omega)}=\sum_{\omega=1}^{n}q^{\text{true}}_\omega f(\x,\omega)$. 
	Here, $\p^*$ depends on the number of observations $N$, the actual observations collected, and also $\x$.  
	The proof of Proposition \ref{prop:weak_conv} implies that $\p^*$ converges in distribution to $\qtrue$ (for all $\x$ including $\x^*$) as $N \rightarrow \infty$.
	Applying this result to Theorem 3.7 of \cite{dupacova1988asymptotic} establishes epiconvergence of $\ee{\p^*}{f(\x,\omega)}$ to $\ee{\qtrue}{f(\x,\omega)}$ under the additional conditions: (i) $X$ is closed and nonempty (satisfied by our assumptions), (ii) for all $\x \in X$, $f(x, \omega)$ is continuous over $\omega$ (satisfied because $\omega$ is discrete), (iii) for all $\omega$, $f(x, \omega)$ is lower-semicontinuous and locally lower Lipschitz on $X$ as defined in \cite{dupacova1988asymptotic} (satisfied by our class of problems \citep{shaDR:09}).
	%, and (iv) convergence in distribution of the $\p^*$ to $\qtrue$ (implied by Proposition \ref{prop:weak_conv}). 
	Consequently, all limit points $\x^*$ of \plp\ solve SLP-2 with probability mass function $\qtrue$. 
	 With $X$ and $\Omega$ compact, the optimal values also converge \citep[]{}.
%we apply Proposition \ref{prop:weak_conv} to \cite[Theorem 3.7]{dupacova1988asymptotic}, which establishes the epiconvergence of (\ref{eq:plp_primal}) under the evident conditions that the objective function (under the worst-case distribution) is continuous with respect to $\omega$ (because $\omega$ is discrete) and lower semicontinuous and locally lower Lipschitz with respect to $x$ (because \plp\ is convex).
    \Halmos
\end{proof}
