...
when $\lambda^*>0$ and $q_\omega >0$. 
	With $\lambda^*>0$ and $q_\omega =0$, we have $p_\omega^* \in \partial \phi^*\left(s_\omega^*\right) q_\omega$ (i.e., $p_\omega^* = 0$) when $s_\omega^* < \bar{s}$, and otherwise (i.e., when $s_\omega^* = \bar{s}<\infty$) we need to use the last two equations in (\ref{eq:p_worst}). 
	With $\lambda^*=0$, $p_\omega^* = 0$ when $h_\omega(\x^*) - \mu^* < 0$; otherwise (when $h_\omega(\x^*) - \mu^* = 0$), we need to use the last two equations in (\ref{eq:p_worst}), but with the regular $\phi$-divergence constraint $I_{\phi}(\p,\q)\leq \rho$ instead of $I_{\phi}(\p,\q)=\rho$. 
%
...
The convexity of \plp\ was noted in \cite{bental2011robust}; see also Proposition \ref{prop:convex} in Section \ref{sec:soln_algorithm}.
...
We provide a proof of Property \ref{property:coherent_risk_measure} in this case in Appendix \ref{sec:apx_proof}. 
...
%
 \begin{APPENDICES}
	...
	\section{Proofs of Select Properties and Propositions}
	\label{sec:apx_proof} 
