...
When this limit is $\infty$ (like the KL divergence), $\phi^*(s)<\infty$ for any finite value of $s$. 
...
We begin in Section~\ref{ssec:suppressandpop} by defining two behaviors---suppression and popping of scenarios---that result in our main classification. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Suppression and Popping of Scenarios: A Main Classification}
...
\begin{itemize}
	\item {\sc Case 1:} ($q_\omega > 0$ but $p_\omega = 0$)
		We call this the ``{\bf Suppression}'' behavior because a scenario with a positive probability in the nominal distribution can take zero probability in the ambiguous problem. 
...
\begin{itemize}
	\item {\sc Subclass 1\ ($\lim_{t \searrow 0} \phi'(t) > -\infty$)}  In this case, there are two nonpositive constants $c, \underline{s}$ such that (i)  $\lim_{t \searrow 0} \phi'(t) = \underline{s}$, and (ii) $\phi^*(s) = c$ for all $s < \underline{s}$. 
		...
		In other words, all scenarios $\omega$ that satisfy the relation $\frac{h_\omega(\x^*)-\mu^*}{\lambda^*} < \underline{s}$ are suppressed.
		As $\rho$ increases, scenarios tend to be suppressed one at a time as their $s_\omega^*$ reaches $\underline{s}$.


	\item {\sc Subclass 2\ ($\lim_{t \searrow 0} \phi'(t) = -\infty$)} In this case, $\phi^*(s) \searrow c$ as $s \rightarrow -\infty$ asymptotically, but never reaches the bound.
...
In this case, $\phi^*(s) \searrow c$ as $s \rightarrow -\infty$ asymptotically, but never reaches the bound.
		As an example of a phi-divergence in this subcategory, see Table~\ref{tb:phi_definitions} and consider the Kullback-Liebler Divergence. 
		For the Kullback-Liebler Divergence, $\lim_{s \rightarrow -\infty}\phi^*(s) =c=-1$. 
		Because such a constant $c$ is reached only as $s \rightarrow -\infty$, scenarios can only be suppressed if $s_\omega = -\infty$, which can only occur if $\lambda = 0$ and $h_\omega(\x) < \mu$.
		Consequently, all solutions with $h_\omega(\x) < \mu$ have $p_\omega=0$ and we must have $\mu = \max_\omega h_\omega(\x)$ to ensure that scenarios $\omega \in \arg\max h_\omega(\x)$ are given positive probability so that $p$ is a probability distribution.
		This means that all but the most expensive scenario(s) will vanish simultaneously.
		Divergences of this type can be difficult to deal with numerically when suppression occurs because of the $\lambda = 0$ in the denominator of $s_\omega$.
...
\end{itemize}
		...
		As an example of a phi-divergence in the first subclass, see Table~\ref{tb:phi_definitions}, and consider the Modified $\chi^2$-Distance. 
		For the Modified $\chi^2$-Distance, $c=-1$ and $\underline{s}=-2=\lim_{t \searrow 0} \phi'(t)$. 


		As an example of a phi-divergence in this subcategory, see Table~\ref{tb:phi_definitions} and consider the Kullback-Liebler Divergence. 
		For the Kullback-Liebler Divergence, $\lim_{s \rightarrow -\infty}\phi^*(s) =c=-1$. 
		Because such a constant $c$ is reached only as $s \rightarrow -\infty$, scenarios can only be suppressed if $s_\omega = -\infty$, ... 
