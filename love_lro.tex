...
As noted in \cite{wang2010likelihood}, the LRLP-2 problem can be viewed as a coherent risk measure.
...
\end{enumerate}
LRLP-2 can be written in the form of a risk envelope in the primal form (\ref{eq:lrlp_primal}) with the change of variables $Q_\omega = \frac{p_\omega}{N_\omega/N}$, with $Q_\omega = 0$ when $N_\omega = 0$.
The inner maximization then becomes $\max_{Q \in {\cal Q}} \e{Qh^\dagger(x)}$, where $Q$ and $h^\dagger(x)$ are the random variables defined by $\{Q_\omega\}$ and $\{h^\dagger_\omega(x)\}$, respectively, and $\cal Q$ is replicates constraints (\ref{eq:lrlp_primal_liklihood}) and (\ref{eq:lrlp_primal_probability}) by $\e{\log(Q)} \geq \frac{1}{N} \log \gamma'$ and $\e{Q} = 1$, respectively.
Here, all expectations are taken with respect to the maximum likelihood distribution.
