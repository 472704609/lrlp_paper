...
We also note that \cite{calafiore2007ambiguous}, \cite{hukullback}, and \cite{wang2010likelihood} all use a different naming convention than the one given here, referring to the Likelihood divergence or Burg entropy as the ``Kullback-Leibler (K-L) divergence''---reversing the order of the arguments $p$ and $q$ relative to the notation presented here.
...
\begin{table}
	...
	{\begin{tabular}{lccccc}
		...
		\hline
		Kullback-Leibler (K-L)            & $\phi_{kl}$        & $\phi_b$                        & $t\log t - t + 1$     & $\sum p_\omega \log\left(\frac{p_\omega}{q_\omega}\right)$ & $e^s - 1$ \\
...
Note that for some $\phi$, such as the J-Divergence, have no closed form representation of $\phi^*$, but can be expressed as the sum of other divergences---Burg Entropy and K-L divergence---which allows the dual to be formed, see \citep{bental2011robust} for details.
...
All scenarios $\omega \in \argmax h_\omega(\x)$ must have a positive probability to maintain a probability distribution. 
		%, so that $\frac{\max_\omega h_\omega(\x) - \mu}{\lambda} = \frac{0}{0}$ can be interpreted so that $p_\omega = q_\omega \phi^{*\prime}\left(\tfrac{0}{0}\right) = 1$.
...
Table \ref{tb:phi_categories} lists $\phi$-divergences that belong to these subcategories. We present the one-by-one and simultaneous suppressing behavior numerically for the Modified $\chi^2$-Distance and K-L Divergence, respectively, in Section \ref{sec:comp_results}.
...
To get such a condition, we must consider how $\rho$ changes as additional samples are taken, and use the notation $\rho_N$ in this section to emphasize the dependence on sample size.
...
\begin{proposition}
	...
	\end{equation}
	where $s^*_\omega = \dfrac{h_\omega(\x^*_N) - \mu^*_N}{\lambda^*_N}$ and $(\x^*_N,\mu^*_N,\lambda^*_N)$ solves the $N$-sample problem with $q_\omega = \tfrac{N_\omega}{N}$.
\end{proposition}
...
This convergence requires that the sequence of nominal distributions $q^N$ converge to the true distribution $\qtrue$ in $L^\infty$, a situation that is satisfied if $q^N$ is the empirical distribution.
In the proof, we assume $q^N = \hat{q}^N$.
We begin by showing that the worst-case distribution converges weakly to the true distribution as $N \rightarrow \infty$.

Let the probability space $(\Xi,{\cal F},\P^\infty)$ be the space associated with taking infinitely many samples from the distribution $\qtrue$.
Let $\Xi' \subset \Xi$ be a measure 1 set such that $\Vert \hat{q}^N(\xi) - \qtrue \Vert_\infty \rightarrow 0$.

\begin{proposition} \label{prop:weak_conv}
	Let $\phi(t)$ be nonnegative and have a unique root at $t = 1$.
	Let $p \neq \qtrue$.
	For all $\epsilon > 0$ and $\xi \in \Xi'$ there exists $N'$ such that $\forall N \geq N'$ $I_{\phi}(p,\hat{q}^N) \leq \frac{\rho_0}{N} \Rightarrow \max_\omega |p_\omega - \qtrue_\omega| \leq \epsilon$.
\end{proposition}
...
The unique root requirement, however, is violated for the special case $\phi$ introduced in Section \ref{ssec:special_phi}.

Proposition \ref{prop:weak_conv} shows that the worst-case distributions of (\ref{eq:plp_primal}) converge weakly to $\qtrue$.
...
\begin{theorem}
	...
	Assume that $X$ is compact, and that the second stage problems $h_\omega(\x)$ are primal and dual feasible for every $\x \in X$.
	Then the optimal value of \plp\ (\ref{eq:plp_two_stage}) converges to to that of SLP-2 (\ref{eq:slp_first_stage}) with distribution $p = \qtrue$.
	Furthermore, the limit points of the solutions of \plp\ solve SLP-2 with distribution $p = \qtrue$.
\end{theorem}
...
%
\begin{APPENDIX}{Proofs of Propositions and Theorem}
...
\begin{proof}{\sc Proof of Proposition \ref{prop:value}.}
	...
	The objective of the $N+1$ sample problem for a given $\x$, $\mu$ and $\kappa$ is $\c\x + \mu + \rho_0 \kappa + \sum_{\omega = 1}^n N'_\omega \left[ \kappa \phi^*\left(\frac{h_\omega(\x) - \mu}{(N+1)\kappa} \right) \right]$,	where $N'_\omega$ is the number of observations of $\omega$ after $N+1$ total observations.
	...
	Now we can guarantee (\ref{eq:raw_cond}) is satisfied with the condition $\kappa \sum_{\omega=1}^n \frac{N_\omega}{N} \phi^{*\prime}(s^{N+1}_\omega) s^{N+1}_\omega - \kappa \phi^*\left(\frac{h_{\hat{\omega}}(x) - \mu}{(N+1)\kappa}\right) > 0$, or, rearranging and dividing by $\kappa > 0$,
...
\begin{proof}{\sc Proof of Proposition \ref{prop:weak_conv}.}
	...
	For simplicity, we assume $\epsilon$ is chosen so that $\max_{\omega \notin Z} \qtrue_\omega > \frac{\epsilon}{2}$.
	
	First, we note that $\max_\omega |p_\omega - \qtrue_\omega| \leq \max_\omega |p_\omega - \hat{q}^N_\omega| + \max_\omega |\hat{q}^N_\omega - \qtrue_\omega|$.
