...
\section*{ABSTRACT}
We adapt and extend the likelihood robust optimization method recently proposed by Wang, Glynn, and Ye for newsvendor problem to a more general two-stage setting.
...
LRLP-2 is an ambiguous stochastic program that is modeled a two-stage minimax problem. Stochastic programs with uncertain objective functions have long been studied by applying the minimax approach to an expected cost; see, e.g., \cite{dupacova_87}.
...
The recent work of \citeN{huhong2013} studies similar problems with ambiguous uncertainty either in the objective or the constraints where the uncertainty sets are defined by KL-divergence.
...
The LRO is an attractive data-driven approach because it uses the data directly.
Furthermore, only those data points or scenarios of interest are used directly in the calculations.
