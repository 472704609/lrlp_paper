...
\begin{itemize}
	\item {\sc Subcategory 1\ ($\lim_{t \searrow 0} \phi'(t) > -\infty$).}  There are nonpositive constants $c,\underline{s}$ such that $\phi^*(s) = c$ for all $s < \underline{s}$. 
		Then $\phi^{*\prime}(s_\omega) = 0$ when $s_\omega < \underline{s}$, which suppresses all such scenarios; i.e., all scenarios that satisfy the relation $\frac{h_\omega(\x)-\mu}{\lambda} < \underline{s}$ are suppressed.
	...
	\item {\sc Subcategory 2\ ($\lim_{t \searrow 0} \phi'(t) = -\infty$).} In this case, $\phi^*(s) \searrow c$ as $s \rightarrow -\infty$ asymptotically, but never reaches the bound.
		...
		Consequently, all solutions with $h_\omega(\x) < \mu$ have $p_\omega=0$; and we must have $\mu = \max_\omega h_\omega(\x)$, so that scenarios $\omega \in \argmax h_\omega(\x)$ are given positive probability in order to maintain a probability distribution.
...
\begin{inparaenum}[\itshape (i\upshape)]
	...
	\item exchanges the polyhedral $\phi$-divergence constraints $\tfrac{h_\omega(\x) - \mu}{\lambda} \leq \bar{s}$ for a (potentially much) smaller set of easily generated feasibility cuts, 
	\item thus maintaining linear master and subproblems.
\end{inparaenum}
...
First, we included an $L^\infty$-norm trust region which is scaled up (by a factor of $3$) or down (by a factor of $\tfrac{1}{4}$) when the trust region inhibits finding the optimal solution or when the polyhedral lower approximation is far from the second-stage expected cost, respectively.
...
Such an upper bound can be computed easily by recognizing the ration $\frac{p_\omega}{q_\omega} \leq \frac{1}{\min_\omega q_\omega}$.
The computational upper bound can then be selected so that $\phi^{*\prime}(\bar{s}_\text{comp}) \geq \frac{1}{\min_\omega q_\omega}$.
Note, however, that an artificial upper bound will induce artificial popping behavior if the nominal distribution contains vanishing probability.
